...
function Set-TargetResource
{
    ...
    if($Ensure -eq "Present")
    {
        ...
        {
             #this is the case when iis site name is specified
             $appCmd += "-verb:sync -source:package=$SourcePath -dest:Auto -setParamFile:$ParamFile"           
        }
...
function Test-TargetResource
{
    ...
    if($ParamFile -ne "" -and $Destination -eq "Auto")
    {
        # site is stored in the paramfile if present
        [xml]$fileContents = Get-Content $ParamFile 
        $Destination = $fileContents.parameters.setParameter | Where {$_.name -eq "IIS Web Application Name" } | Select -ExpandProperty value
    }
